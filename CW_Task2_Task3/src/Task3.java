...
public class Task3 {
    ...
    public static void main(String[] args) throws IOException{
        ...
        CircularQueue WaitingList = new CircularQueue(6); //create queue class
        ...
        while (endLoop) { //make loop to run main list continuously
            ...
            switch (Character.toUpperCase(input.next().charAt(0))) {
                case 'A' -> addsCustomerCabin(WaitingList, myShip, myCabin);
                case 'V' -> viewAllCabin(myShip, myCabin);
                case 'E' -> emptyCabins(myShip);
                case 'D' -> deleteCustomerFromCabin(myShip, WaitingList, myCabin);
                case 'F' -> findCabinFromCustomerName(myShip, myCabin);
                case 'S' -> storeProgramDataToFile(myShip, myCabin);
                case 'L' -> loadProgramDataFromFile(myShip, myCabin);
    ...
    public static void addsCustomerCabin(CircularQueue WaitingList, Cabin[] myShip, Passenger[][] myCabin) {
        ...
        if (cabinNum >= 0 && cabinNum < 12) {  //check the cabin number in 0 to 12
            ...
            if (passengerLocation >= 0 && passengerLocation < 3) { // check passenger location in 0 - 3
                ...
                addsCustomerCabin(WaitingList, myShip, myCabin); //use recursion method
            }
        ...
        addQueue(myCabin, WaitingList, myShip); //call addQueue method
    }

    public static void addQueue(Passenger[][] myCabin, CircularQueue WaitingList, Cabin[] myShip) {
        Scanner input = new Scanner(System.in);

        for (int i = 0; i < 1; i++) {
            //check all the cabin have at least one passenger
            if (myShip[i].getcabinNum() != 20 && myShip[i + 1].getcabinNum() != 20 && myShip[i + 2].getcabinNum() != 20 && myShip[i + 3].getcabinNum() != 20
                    && myShip[i + 4].getcabinNum() != 20 && myShip[i + 5].getcabinNum() != 20 && myShip[i + 6].getcabinNum() != 20 && myShip[i + 7].getcabinNum() != 20
                    && myShip[i + 8].getcabinNum() != 20 && myShip[i + 9].getcabinNum() != 20 && myShip[i + 10].getcabinNum() != 20 && myShip[i + 11].getcabinNum() != 20) {

                System.out.println("""
                        Do you want to add person to queue:
                        y: Yes
                        N: No\s""");
    ...
    public static void deleteCustomerFromCabin(Cabin[] myShip, CircularQueue WaitingList, Passenger[][] myCabin){
        Scanner input = new Scanner(System.in);
        ...
        if (WaitingList.endName() != null){
            myShip[cabinNum].setcabinNum(cabinNum); //make cabin number as input cabin number
            myCabin[cabinNum][0].setFirstName(WaitingList.firstName()); //assign cabin number first name
            System.out.println(WaitingList.firstName() +  " Enter your last name ");
            myCabin[cabinNum][0].setSurName(input.next()); //input surname
            int expenses = 0;
            do {
                try {
                    System.out.println(WaitingList.firstName() + " Enter your Expense ");
                    expenses = input.nextInt(); //input expenses
                    myCabin[cabinNum][0].setExpenses(expenses);
            ...
            WaitingList.DeQueue(); //dequeue from queue array
        }
    }
    ...
    public static void findCabinFromCustomerName(Cabin[] myShip, Passenger[][] myCabin){
        ...
        for(int i= 0; i < myShip.length; i++){
            ...
            for (int k = 0; k < myCabin[i].length; k++){
                ...
                if (passengerName.equalsIgnoreCase(myCabin[i][k].getFirstName()) && !checkCustomerName){
    ...
    public static void storeProgramDataToFile(Cabin[] myShip, Passenger[][] myCabin) throws IOException{
        try (PrintWriter out = new PrintWriter(new FileWriter("C:/Users/HP/Desktop/Folder/IIT/1st Year 2nd Sem/SD 2/CW/CW_Task2_Task3/Data/Task3.txt"))) {
            for (int i = 0; i < myShip.length; i++){
    ...
    public static void loadProgramDataFromFile(Cabin[] myShip, Passenger[][] myCabin) throws IOException{

        //call the load file
        try {
            File filelocation = new File("C:/Users/HP/Desktop/Folder/IIT/1st Year 2nd Sem/SD 2/CW/CW_Task2_Task3/Data/Task3.txt");
